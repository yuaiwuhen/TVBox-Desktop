...
async function testWexGuaZiSpider() {
  ...
  console.log('\n[1] 检查目录结构:');
  console.log(
    `  jar_cache: ${jarCacheDir} - ${fs.existsSync(jarCacheDir) ? '✓存在' : '✗不存在'}`,
  );
  console.log(
    `  WexGuaZiGuard: ${guaziDir} - ${fs.existsSync(guaziDir) ? '✓存在' : '✗不存在'}`,
  );
  console.log(
    `  libLoadNiMa.so: ${libPath} - ${fs.existsSync(libPath) ? '✓存在' : '✗不存在'}`,
  );

  // 检查 stub JAR
  const stubJar = path.join(
    __dirname,
    'tools',
    'tvbox-spider-stubs-complete.jar',
  );
  console.log(
    `  stub JAR: ${stubJar} - ${fs.existsSync(stubJar) ? '✓存在' : '✗不存在'}`,
  );
  ...
  console.log(
    `  wexguard JAR: ${wexguardJar} - ${fs.existsSync(wexguardJar) ? '✓存在' : '✗不存在'}`,
  );
  ...
  console.log('\n问题:');
  console.log(
    '  如果 stub JAR 的 LoadNiMa.class被加载，它会替代wexguard中的原始类',
  );
