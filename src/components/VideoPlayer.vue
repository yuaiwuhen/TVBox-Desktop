...
const toggleAppFullscreen = async () => {
  try {
    // 调用 Electron IPC 最大化窗口
    const electronIPC = (window as any).electronIPC;
    if (electronIPC) {
      await electronIPC.invoke('window-toggle-maximize');
      isAppFullscreen.value = !isAppFullscreen.value;
      console.log('[VideoPlayer-hevc] 应用全屏切换');
    } else {
      console.warn('[VideoPlayer-hevc] Electron IPC not available');
    }
  } catch (error) {
    console.error('[VideoPlayer-hevc] ❌ 应用全屏失败:', error);
  }
};
...
<template>
  <div ref="playerContainer" class="video-player-wrapper relative w-full h-full bg-black select-none" tabindex="0"
    @keydown="onKeyDown" @mousedown="onMouseDown" @mouseup="onMouseUp" @dblclick="onDoubleClick"
