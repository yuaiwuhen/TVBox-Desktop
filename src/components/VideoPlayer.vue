...
// Seeking state — prevents timeupdate from overriding slider during user drag
...
function initPlayer(url: string) {
  ...
  if (!playerContainer.value || !url) {
    console.warn('[VideoPlayer] 容器或URL为空，跳过初始化');
    return
  }
  ...
  console.log(`[VideoPlayer] 视频格式: m3u8=${isM3u8}, flv=${isFlv}, dash=${isDash}`);
  ...
  playerInstance.on('playing', () => {
    console.log('%c[VideoPlayer] ✅ 播放成功', 'color: green; font-weight: bold;');
  ...
  playerInstance.on('canplaythrough', () => {
    isBuffering.value = false
  })
  ...
  playerInstance.on('error', (err: any) => {
    ...
    console.error('[VideoPlayer] 错误详情:', err);
    isBuffering.value = false
...
// --- Controls ---
function togglePlay() {
  console.log('[VideoPlayer] togglePlay clicked, playerInstance=', !!playerInstance, 'isPlaying=', isPlaying.value)
...
function toggleMute() {
  console.log('[VideoPlayer] toggleMute clicked')
  if (!playerInstance) return
...
function togglePiP() {
  console.log('[VideoPlayer] togglePiP clicked, playerInstance=', !!playerInstance)
  if (!playerInstance) return
  try {
...
function skipForward() {
  console.log('[VideoPlayer] skipForward clicked, timeStep=', timeStep.value)
  if (!playerInstance) return
  playerInstance.currentTime = Math.min((playerInstance.currentTime || 0) + timeStep.value, duration.value)
}

function skipBackward() {
  console.log('[VideoPlayer] skipBackward clicked, timeStep=', timeStep.value)
  if (!playerInstance) return
...
function toggleDanmu() {
  console.log('[VideoPlayer] toggleDanmu clicked, danmuEnabled=', danmuEnabled.value)
...
function onSearchSubtitleClick() {
  console.log('[VideoPlayer] searchSubtitle clicked')
  emit('searchSubtitle')
}
...
.video-player-wrapper :deep(.xgplayer video) {
  z-index: 0 !important;
  pointer-events: none;
}
